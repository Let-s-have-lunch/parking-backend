...
import com.lineacademy.parkingbackend.config.JwtProvider;
...
public class UserService {
    ...
    /**
     * 로그인 기능
     * 반환값: Tuple2<String (토큰), User (회원정보)>
     */
    ...
    public Mono<Tuple2<String, User>> login(String email, String rawPassword) {
        return Mono.fromCallable(() -> userRepository.findByEmail(email)
                ...
                .flatMap(user -> {
                    ...
                    // Reactor의 Tuples를 사용하여 토큰과 User 객체를 함께 묶어서 반환
        ...
        return Mono.fromCallable(() -> userRepository.existsByEmail(email))
                ...
                .flatMap(exists -> {
                    ...
                    User newUser = User.builder()
                            ...
                            .nickname(nickname)
                            .role(UserRole.USER) // 혹은 UserRole 설정 방식에 맞게
                            .build();
